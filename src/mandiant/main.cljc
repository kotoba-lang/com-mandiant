(ns mandiant.main
  "Kotodama WASM entrypoint for the Mandiant clean-room actor (L4) — Clojure port.
  ...
  facts (`mandiant.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mandiant")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mandiant.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mandiant-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mandiant-compat/ready)
